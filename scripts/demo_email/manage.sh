...
# a switchable 'Demo' account in Firehouse + fake seed mail.
#
...
DOCKER_DIR="${FIREHOUSE_DEMO_MAIL_DIR:-$HOME/docker/snappymail}"
USERS_FILE="$DOCKER_DIR/dovecot/conf/users"
DEMO_USER="demo@firehouse.local"
DEMO_PASS="demodemo"
...
case "${1:-}" in
  setup)
    ...
    echo
    echo "Done. In Firehouse, switch to the 'Demo' account to show off the inbox."
    ;;
  reseed)
    "$PY" "$HERE/seed_demo_emails.py" --reset
    ;;
