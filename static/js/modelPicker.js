...
function _initModelPickerDropdown() {
  ...
  function _populate(filter) {
    ...
    // Load favorites
    const favs = (function() { try { return JSON.parse(localStorage.getItem('firehouse-model-favorites') || '[]'); } catch { return []; } })();
  ...
  async function _pick(m) {
    ...
    try { document.dispatchEvent(new CustomEvent('firehouse:model-picked', { detail: m })); } catch {}
  ...
  document.addEventListener('firehouse:auto-select-model', async (e) => {
...
export function updateModelPicker() {
  ...
  // SECURITY: deliberately NOT auto-injecting `firehouse-model-favorites[0]`
