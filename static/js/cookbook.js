...
async function _fetchDependencies() {
  ...
  try {
    ...
    list.innerHTML = [
      _viewingRemote ? '' : _section('Firehouse app', 'Run inside the Firehouse app itself.', _appDeps),
...
function _renderRecipes() {
  ...
  html += '</select>';
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Optional packages that extend Firehouse capabilities.</p>';
  ...
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Configure SSH servers, install Firehouse keys, choose model directories, and set the default server. Local is this machine.</p>';
